...
function fetchQuestionData(examName, currentProblem) {
    const spreadsheetId = '1yEb5m_fjw3msbBYLFtO55ukUI0C0XkJfLurWWyfALok';
    const range = '문항정보!A2:E';  // 문항정보 시트의 A2:E 열까지 가져옵니다.
    ...
    gapi.client.sheets.spreadsheets.values.get({
        ...
        const data = response.result.values;
        if (data) {
            const filteredData = data.filter(row => row[1] === examName);
            const numProblems = filteredData.length;
            renderProblemNavigation(numProblems, currentProblem, examName);
        }
    }).catch(error => {
...
function loadMenuData() {
    // 메뉴 데이터를 가져오고, 메뉴 클릭 시 해당 시험지의 문제를 로드하도록 합니다.
    const spreadsheetId = '1yEb5m_fjw3msbBYLFtO55ukUI0C0XkJfLurWWyfALok';
    const range = '문항정보!A2:E';
    ...
    gapi.client.sheets.spreadsheets.values.get({
        ...
        if (data) {
            ...
            data.forEach(row => {
                ...
                examMap.get(examName).push(row);
            });

            examMap.forEach((problems, examName) => {
                const examItem = document.createElement('li');
                examItem.textContent = examName;
                examItem.classList.add('menu-item');
                examItem.addEventListener('click', function () {
                    loadProblem(1, examName);  // 첫 번째 문제를 로드
                });
                navList.appendChild(examItem);
            });
        }
